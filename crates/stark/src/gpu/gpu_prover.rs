...
use p3_field::{PrimeField, PackedValue};
...
use crate::baby_bear_poseidon2::{Challenge, Val, MyHash, MyCompress, Challenger};
use crate::gpu::ffi::*;
use crate::InnerChallenge;
use crate::{GpuMerkleProverData,  GpuMerkleTreeHandle,
...
impl CudaResultCheck for i32 {
    ...
    fn check(self, msg: &str) {
        if self != 0 { panic!("CUDA FFI Error: {} (code {})", msg, self); }
    }
}
...
where
    Challenger: FieldChallenger<Val> + CanObserve<<GpuChallengeMmcs as Mmcs<Challenge>>::Commitment>,
{
    ...
    unsafe {
        cuda_malloc(&mut d_current_evals, initial_size_bytes).check("Initial commit_phase cuda_malloc failed");
        cuda_memcpy_htod(d_current_evals, initial_evals.as_ptr() as *const c_void, initial_size_bytes).check("Initial commit_phase cuda_memcpy_htod failed");
    }
    ...
    for log_folded_height in (config.log_blowup..log_max_height).rev() {
        if d_current_evals.is_null() { break; }
        ...
        let prover_data_for_layer: GpuMerkleProverData<p3_matrix::extension::FlatMatrixView<Val, Challenge, RowMajorMatrix<Challenge>>> = GpuMerkleProverData {
        ...
        let final_prover_data = unsafe {
             std::mem::transmute(prover_data_for_layer)
        };
        data.push(final_prover_data);
        
        if current_num_evals == 1 { break; }
        ...
        unsafe { cuda_free(d_current_evals).check("cuda_free of old layer failed"); }
...
pub fn answer_queries_gpu(
    ...
    commit_phase_data: &[<GpuChallengeMmcs as Mmcs<Challenge>>::ProverData<RowMajorMatrix<Challenge>>],
    commit_phase_layer_evals: &[Vec<Challenge>],
    query_indices: &[usize],
) -> Vec<QueryProof<Challenge, GpuChallengeMmcs>> {
    if query_indices.is_empty() { return Vec::new(); }
    ...
    for &index in query_indices {
        for layer_idx in 0..commit_phase_data.len() {
            ...
            queries_by_layer.entry(layer_idx).or_default().entry(index_pair).or_default().push(index);
        }
    }
    ...
    for (layer_idx, pairs) in &queries_by_layer {
        ...
        let cuda_prover_data = unsafe {
            &*(data as *const _ as *const GpuMerkleProverData<()>)
        };
    ...
    if result != 0 { panic!("stark_fri_generate_proofs_gpu FFI call failed"); }
...
mod tests {
    ...
    use crate::baby_bear_poseidon2::{MyHash, MyCompress, my_perm};
    ...
    fn test_fri_commit_on_gpu() {
        ...
        let (cpu_root, _cpu_prover_data) = cpu_fri_mmcs.commit_matrix(RowMajorMatrix::new(evals.clone(), 2));
        let duration_cpu = start_cpu.elapsed();
        ...
        if result != 0 {
            ...
            unsafe { cuda_free(d_evals).check("Test: cuda_free (on error) failed"); }
        ...
        assert_eq!(cpu_root, gpu_root, "The GPU-computed Merkle root for the FRI layer does not match the CPU reference!");
    ...
    fn test_fri_query_gpu() {
        ...
        let start_cpu = std::time::Instant::now();
        // `ExtensionMmcs` expects a `Matrix`. We treat our `evals` Vec as a single-column matrix.
        let (cpu_root, cpu_prover_data) = cpu_fri_mmcs.commit_matrix(RowMajorMatrix::new(evals.clone(), 2));
        let duration_cpu = start_cpu.elapsed();
        println!("CPU commit finished in {:?}", duration_cpu);
        ...
        let index_i_sibling = index_i ^ 1;
        let index_pair = index_i >> 1;
        println!("*****cpu_answer_query: index:{}, index_i_sibling:{},index_pair:{} ", index, index_i_sibling, index_pair);
        let (mut opened_rows, opening_proof) = cpu_fri_mmcs.open_batch(index_pair, &cpu_prover_data).unpack();
        ...
        assert_eq!(opening_proof, query_proofs[0].commit_phase_openings[0].opening_proof, "The GPU opening_proof not match the CPU !");
        assert_eq!(sibling_value, query_proofs[0].commit_phase_openings[0].sibling_value, "The GPU sibling_value not match the CPU !");
    ...
    fn test_first_fri_commit_consistency() {
        ...
        let (expected_root, _) = cpu_mmcs.commit_matrix(
            RowMajorMatrix::new(initial_evals.clone(), 2)
        );
