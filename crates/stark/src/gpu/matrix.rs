...
use crate::{
    GpuMemBlk,
    baby_bear_poseidon2::{Challenge, Val},
};
use std::fmt;
...
impl CudaResultCheck for i32 {
    ...
    fn check(self, msg: &str) -> Result<(), Self::Error> {
        if self == 0 {
            ...
            Err(CudaError { code: self, message: msg.to_string() })
        }
    }
}
...
impl<T> GpuMatrix<T> {
    ...
    pub unsafe fn from_raw_parts(ptr: *mut c_void, width: usize, height: usize) -> Self {
        Self { ptr, width, height, _marker: PhantomData }
    }
    ...
    pub fn row(&self, row_idx: usize) -> Result<Vec<T>, CudaError> {
        ...
        let device_ptr_offset =
            unsafe { (self.ptr as *mut u8).add(row_idx * self.width * element_size) };
    ...
    pub fn copy_from_host(&self, host_vec: &[T]) {
        ...
        if size_bytes > 0 {
            unsafe {
                let _ = cuda_memcpy_htod(self.ptr, host_vec.as_ptr() as *const c_void, size_bytes)
    ...
    pub fn to_host(&self) -> Vec<T> {
        ...
        unsafe {
            ...
            let _ = cuda_memcpy_dtoh(host_vec.as_mut_ptr() as *mut c_void, self.ptr, size_bytes)
    ...
    pub fn from_vec(
        ...
        assert_eq!(
            host_vec.len(),
            height * width,
            "Vector length does not match matrix dimensions"
        );
...
impl GpuMatrix<Challenge> {
    pub fn flatten_to_base(&self, gpu_mem_blk: &GpuMemBlk) -> GpuMatrix<Val> {
        ...
        let flat_matrix = gpu_mem_blk.alloc_matrix::<Val>(self.height, base_width);

        unsafe {
            let _ = flatten_permutation_trace_gpu(
...
#[cfg(test)]
mod tests {
    use crate::baby_bear_poseidon2::{Challenge, StarkConfigCpu, Val};
    use crate::gpu::ffi::*;
    ...
    fn test_flatten_permutation_trace_gpu() {
        ...
        let source_values_challenge: Vec<Challenge> =
            (0..height * width).map(|_| rng.r#gen()).collect();
        ...
        let expected_flat_matrix_cpu: RowMajorMatrix<Val> =
            source_matrix_cpu.clone().flatten_to_base();
        ...
        let flat_matrix_gpu_handle: GpuMatrix<Val> =
            source_matrix_gpu.flatten_to_base(&gpu_mem_blk);
        ...
        assert_eq!(
            actual_flat_values_gpu.len(),
            height * expected_width,
            "Downloaded vector has incorrect length"
        );

        // b) Verify data content
        assert_eq!(
            expected_flat_matrix_cpu.values, actual_flat_values_gpu,
    ...
    fn test_split_matrix_gpu() {
        ...
        let source_domain =
            <MyPcs as Pcs<Challenge, Challenger>>::natural_domain_for_degree(config.pcs(), height);
        ...
        let source_matrix_gpu =
            GpuMatrix::from_vec(&source_matrix_cpu.values, height, width, &gpu_mem_blk);
        ...
        for i in 0..num_chunks {
            ...
            assert_eq!(
                cpu_chunk.height(),
                gpu_chunk_handle.height,
                "Height mismatch for chunk {}",
                i
            );
            assert_eq!(cpu_chunk.width(), gpu_chunk_handle.width, "Width mismatch for chunk {}", i);

            // Compare data
            assert_eq!(cpu_chunk.values, gpu_chunk_values, "Data mismatch for chunk {}", i);
        }
