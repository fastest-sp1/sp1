...
use p3_field::{PrimeCharacteristicRing, ExtensionField, Field, PrimeField32};
use p3_matrix::{
...
pub fn debug_cumulative_sums<F: Field, EF: ExtensionField<F>>(perms: &[RowMajorMatrix<EF>]) {
    let sum: EF = perms.iter().map(|perm| *perm.row_slice(perm.height() - 1).unwrap().into_iter().last().unwrap()).sum();
