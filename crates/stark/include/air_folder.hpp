...
__device__ inline void folder_assert_zero(ProverConstraintFolder<Challenge>& folder, Val val) {
...
__device__ inline void folder_assert_zero_ext(ProverConstraintFolder<Challenge>& folder, Challenge challenge) {
    int local_idx = (*folder.constraint_index)++;
...
__device__ inline void folder_assert_bool(ProverConstraintFolder<Challenge>& folder, Val val) {
...
__device__ inline void folder_assert_ext_eq(ProverConstraintFolder<Challenge>& folder, Challenge left, Challenge right, Val factor) {
    for (int i =0; i < 4; ++i){
