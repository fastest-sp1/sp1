...
__device__ void eval_ext_alu_chip(
    ...
    //const int perm_width_ef = perm_width / 4; // Width in Challenge elements
    ...
    const auto* main_cols = reinterpret_cast<const ExtAluCols<Val>*>(main_local_row);
    const auto* prep_cols = reinterpret_cast<const ExtAluPreprocessedCols<Val>*>(prep_local_row);
    ...
    for (int i = 0; i < 4; ++i) {
         const auto& vals = main_cols->values[i].vals;
          ...
          VirtualPairCol vpc_is_add = VirtualPairCol::single_preprocessed(offsetof(ExtAluPreprocessedCols<Val>, accesses[i].is_add) / sizeof(Val));
          VirtualPairCol vpc_is_sub = VirtualPairCol::single_preprocessed(offsetof(ExtAluPreprocessedCols<Val>, accesses[i].is_sub) / sizeof(Val));
          VirtualPairCol vpc_is_mul = VirtualPairCol::single_preprocessed(offsetof(ExtAluPreprocessedCols<Val>, accesses[i].is_mul) / sizeof(Val));
          VirtualPairCol vpc_is_div = VirtualPairCol::single_preprocessed(offsetof(ExtAluPreprocessedCols<Val>, accesses[i].is_div) / sizeof(Val));
          ...
          folder_assert_ext_eq(folder, bb31_quartic_extension_t::from_block(vals.in1) + bb31_quartic_extension_t::from_block(vals.in2), bb31_quartic_extension_t::from_block(vals.out), access.is_add);
          folder_assert_ext_eq(folder, bb31_quartic_extension_t::from_block(vals.in1), bb31_quartic_extension_t::from_block(vals.in2) + bb31_quartic_extension_t::from_block(vals.out), access.is_sub);
