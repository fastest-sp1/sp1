...
fn main() {
    ...
    let target_dir = {
        ...
        loop {
            ...
            if !dir.pop() {
                ...
                println!("cargo:warning=Could not find 'target' directory in OUT_DIR path. IDE header symlinks might not be created.");
                break out_dir.clone(); 
            }
        }
    };
    ...
    if !core_machine_include_path.exists() {
        panic!("Could not find sp1-core-machine include directory at hardcoded path: {:?}", core_machine_include_path);
    }
    ...
    let cuda_sources = glob::glob(crate_dir.join(SOURCE_DIRNAME).join("**/*.cpp").to_str().unwrap())
        .unwrap()
        .map(|p| p.unwrap())
        .collect::<Vec<_>>();
    ...
    if cuda_sources.is_empty() {
             println!("cargo:warning=CUDA feature is enabled, but no .cu files were found in the '{}' directory.", SOURCE_DIRNAME);
             return;
    }
    ...
    for cu_file in &cuda_sources {
        ...
        if !output.status.success() {
                eprintln!("nvcc compilation stdout for {}:\n{}", cu_file.display(), String::from_utf8_lossy(&output.stdout));
                eprintln!("nvcc compilation stderr for {}:\n{}", cu_file.display(), String::from_utf8_lossy(&output.stderr));
    ...
    ar_command.arg("rcs") // r: insert with replacement, c: create if not exists, s: write index
    ...
    Q: AsRef<Path>,
{
    ...
    if let Err(e) = os::unix::fs::symlink(diff_paths(&original, target_dir).unwrap(), &link) {
        ...
        println!("cargo:warning=Failed to create symlink from {:?} to {:?}: {}", original.as_ref(), link.as_ref(), e);
    }
}