...
impl<C: Config> Builder<C> {
    ...
    pub fn exp_e_bits(
        ...
        let mut result = self.eval(SymbolicExt::from_ext(C::EF::ONE));
    ...
    pub fn ext_from_base_slice(&mut self, arr: &[Felt<C::F>]) -> Ext<C::F, C::EF> {
        ...
        let mut res = SymbolicExt::from_ext(C::EF::ZERO);
        for i in 0..arr.len() {
            res += arr[i] * SymbolicExt::from_ext(C::EF::ith_basis_element(i).unwrap());
        }
        self.eval(res)
    }
