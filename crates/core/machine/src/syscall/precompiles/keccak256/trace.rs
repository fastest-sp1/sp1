...
impl<F: PrimeField32> MachineAir<F> for KeccakPermuteChip {
    ...
    fn generate_trace(
        ...
        let dummy_keccak_rows = generate_trace_rows::<F>(vec![[0; STATE_SIZE]], LOG_BLOWUP);
        let mut dummy_chunk = Vec::new();
        for i in 0..NUM_ROUNDS {
            let dummy_row = dummy_keccak_rows.row(i).unwrap();
            let mut row = [F::ZERO; NUM_KECCAK_MEM_COLS];
            row[..NUM_KECCAK_COLS].copy_from_slice(dummy_row.into_iter().collect::<Vec<_>>().as_slice());
...
impl KeccakPermuteChip {
    pub fn populate_chunk<F: PrimeField32>(
        ...
        let p3_keccak_trace = generate_trace_rows::<F>(vec![event.pre_state], LOG_BLOWUP);

        // Create all the rows for the permutation.
        for i in 0..NUM_ROUNDS {
            let p3_keccak_row = p3_keccak_trace.row(i).unwrap();
            let row = &mut chunk[i * NUM_KECCAK_MEM_COLS..(i + 1) * NUM_KECCAK_MEM_COLS];
            // Copy p3_keccak_row into start of cols
            row[..NUM_KECCAK_COLS].copy_from_slice(p3_keccak_row.into_iter().collect::<Vec<F>>().as_slice());
            let cols: &mut KeccakMemCols<F> = row.borrow_mut();

            cols.shard = F::from_u32(shard);
            cols.clk = F::from_u32(start_clk);
