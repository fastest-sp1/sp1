...
template<typename F>
struct BatchFRIInstrRowFFI {
  Address_C<F> base_vec_addrs_p_at_x;
  Address_C<F> ext_single_addrs;
  Address_C<F> ext_vec_addrs_p_at_z;
  Address_C<F> ext_vec_addrs_alpha_pow;
  F acc_mult;
  F is_end;
};
...
struct ExpReverseBitsEventFlatFFI {
  F base_val;
  uint32_t exp_bits_offset;
  uint32_t exp_len;
  F result_val;
};

template<typename F>
struct ExpReverseBitsEventRowFFI {
  F base;
  F exp;
  F result;
  F accum;
...
template<typename F>
struct ExpReverseBitsInstrRowFFI {
  Address<F> base;
  Address<F> exp;
  Address<F> result;
  F mult;
